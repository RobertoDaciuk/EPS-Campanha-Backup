...
export class AutenticacaoService {
  ...
  async registrar(dados: RegistrarUsuarioDto): Promise<{ message: string }> {
    ...
    const usuario = await this.prisma.usuario.create({
      data: {
        ...
        papel: 'VENDEDOR', // Hardcoded: auto-registro é sempre VENDEDOR
        status: 'PENDENTE', // Hardcoded: aguarda aprovação do Admin
      },
    });
  ...
  async login(dados: LoginDto): Promise<RespostaLogin> {
    ...
    return this.gerarToken({  // <-- Mudou de _gerarToken para gerarToken
      id: usuario.id,
      email: usuario.email,
      papel: usuario.papel,
      nome: usuario.nome,
