 ...
 * - Adicionada verificação `(dados.totalMoedinhasTime ?? 0)`
 * para prevenir o erro `toLocaleString` em valores nulos/indefinidos.
 *
 * @module Dashboard
 * ============================================================================
 */
...
export function KpisGerente({ dados }: KpisGerenteProps) {
  ...
  return (
    ...
    >
      ...
      />
      <KpiCard
        titulo="Moedinhas (Equipe)"
        // CORRIGIDO: Adicionado `?? 0` para robustez (Princípio 1)
        valor={(dados.totalMoedinhasTime ?? 0).toLocaleString("pt-BR")}
        descricao="Total acumulado pela sua equipe"
        Icone={Gem}
        cor="primary"
      />
    </motion.div>
